...
const SearchBoxWithSelect = (props: props) => {
    const {searchPlaceHolder, selectPlaceHolder} = props;
    return (
        <div className="flex gap-2 p-2 divide-x-2 rounded-md rtl:flex-row-reverse bg-light-2 dark:bg-dark-6 divide-dark-6 dark:divide-dark-4">
            <div className="flex gap-2 rtl:flex-row-reverse basis-1/4">
                <MapIcon className="h-6 2-6 text-dark-6 dark:text-dark-4" />
                <span className="text-dark-6 dark:text-dark-4">{selectPlaceHolder}</span>
            </div>
            <div className="flex justify-end gap-2 rtl:flex-row-reverse basis-3/4">
